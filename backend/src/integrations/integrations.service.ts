...
export class IntegrationsService {
  ...
  async compileTemplate(content: string, lead: any, agent?: any) {
    ...
    compiled = compiled.replace(/\{\{agentName\}\}/g, agent?.firstName ? `${agent.firstName} ${agent.lastName || ''}`.trim() : 'RENS Representative');
    compiled = compiled.replace(/\{\{agentEmail\}\}/g, agent?.email || 'support@rensecosystem.com');
    return compiled;
  }
  ...
  async simulateEmail(organizationId: string, leadId: string, templateId: string, customSubject?: string, customBody?: string) {
    ...
    let subject = customSubject || 'RENS Ecosystem Real Estate';
    let body = customBody || '';
    ...
    const payload = {
      ...
      from: lead.assignedTo?.email || 'agent@rensecosystem.com',
      subject,
      body,
    };
  ...
  async triggerVapiCall(organizationId: string, leadId: string) {
    ...
    const payload = {
      ...
      variableValues: {
        leadName: lead.name,
        agentName: lead.assignedTo?.firstName || 'RENS Representative',
      },
    };
    ...
    if (isLive) {
      ...
      const transcripts = [
        ...
        `[Vapi AI]: Hi, this is the AI assistant calling on behalf of RENS Real Estate Ecosystem. I saw you recently showed interest in our active listings in Dubai. I wanted to ask if you are actively looking to buy or rent?`,
        ...
        `[Vapi AI]: Perfect! I have flagged your preference. An elite Realtor from RENS, ${lead.assignedTo?.firstName || 'our agent team'}, will send you exclusive, unlisted options directly to your email shortly. Does that work?`,
  ...
  async getPortalsXmlFeed(organizationId: string) {
    ...
    properties.forEach((prop) => {
      xml += `  <property>\n`;
      xml += `    <reference_number>RENS-${prop.id.substring(0, 8).toUpperCase()}</reference_number>\n`;
      ...
      xml += `    <agent>\n`;
      xml += `      <name>${prop.assignedTo?.firstName || 'RENS Team'} ${prop.assignedTo?.lastName || ''}</name>\n`;
      xml += `      <email>${prop.assignedTo?.email || 'info@rensecosystem.com'}</email>\n`;
      xml += `    </agent>\n`;
      xml += `    <owner>\n`;
  ...
  async simulateDriveSync(organizationId: string, documentId: string) {
    ...
    const payload = {
      ...
      driveFolder: `RENS_ERP_SYNC/${document.category.toUpperCase()}`,
    };
