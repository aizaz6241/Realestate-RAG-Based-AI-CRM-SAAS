...
export class AiDatabaseToolsService {
    ...
    try {
      switch (toolName) {
        ...
        case 'createTask': {
          ...
          this.zorvexGateway.broadcastToOrganization(organizationId, 'task_sync', {
            action: 'create',
            task: {
              id: task.id,
              title: task.title,
        ...
        case 'sendReminder': {
          ...
          this.zorvexGateway.broadcastToOrganization(organizationId, 'alert_sync', {
            action: 'create',
            message: messageText,
        ...
        case 'generateEnterpriseReport': {
          ...
          try {
            ...
            return {
              ...
              message: `Successfully generated premium executive Zorvex Operational ${typeUpper} report. You can download or view it at ${downloadUrl}`
            };
          } catch (err) {
  ...
  private generateBrandedHtmlReport(reportType: string, data: any, organizationId: string): string {
    ...
    const filename = `Zorvex_Enterprise_${reportType}_${Date.now()}.html`;
  ...
  private buildReportHtml(reportType: string, data: any, organizationId: string): string {
    ...
    let reportTitle = 'Zorvex Enterprise Operations Report';
    ...
    if (reportType === 'FINANCE') {
      ...
      reportTitle = 'Zorvex Financial Operations & Payroll Report';
      summaryCardsHtml = `
        <div class="card">
      ...
      reportTitle = 'Zorvex Real Estate Inventory & Assets Report';
      summaryCardsHtml = `
        <div class="card">
          <div class="card-icon" style="background: rgba(16, 185, 129, 0.1); color: #10B981;">🏢</div>
      ...
      reportTitle = 'Zorvex Enterprise Task Board & Kanban Report';
      summaryCardsHtml = `
        <div class="card">
  ...
  <div class="container">
    <header>
      ...
      <div class="meta-section">
        ...
        <p>Organization Context: <strong>Zorvex Portal</strong></p>
      </div>
    </header>
    ...
    <footer>
      <p>&copy; ${new Date().getFullYear()} Zorvex ERP. Confidential Operations Document. Powered by Zorvex-AOS 5.0 Orchestrator.</p>
    </footer>
  </div>
</body>
</html>
